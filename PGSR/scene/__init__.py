...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, exclude_ply_path=None, load_iteration=None, shuffle=True,
        ...
        self.multi_view_num = args.multi_view_num if hasattr(args, 'multi_view_num') else 8
        for resolution_scale in resolution_scales:
            ...
            with open(os.path.join(self.model_path, "multi_view.json"), 'w') as file:
                for id, cur_cam in enumerate(self.train_cameras[resolution_scale]):
                    ...
                    multi_view_max_angle = args.multi_view_max_angle if hasattr(args, 'multi_view_max_angle') else 30
                    multi_view_min_dis = args.multi_view_min_dis if hasattr(args, 'multi_view_min_dis') else 0.01
                    multi_view_max_dis = args.multi_view_max_dis if hasattr(args, 'multi_view_max_dis') else 1.5
        ...
        if self.loaded_iter:
            if exclude_ply_path is None:
                self.gaussians.load_ply(os.path.join(self.model_path,
                                                     "point_cloud",
                                                     "iteration_" + str(self.loaded_iter),
                                                     "point_cloud.ply"))
            else:
                self.gaussians.load_ply(exclude_ply_path)
        else:
