...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background,
               app_model=None, max_depth=5.0, volume=None, use_depth_filter=False, mask_path=None):
    ...
    if volume is not None:
        ...
        for idx, view in enumerate(tqdm(views, desc="TSDF Fusion progress")):
            ...
            if use_depth_filter and len(view.nearest_id) > 2:
                ...
                pts_in_nearest_cam = torch.matmul(
                    nearest_world_view_transforms[:, None, :3, :3].expand(num_n, H * W, 3, 3).transpose(-1, -2),
                    pts[None, :, :, None].expand(num_n, H * W, 3, 1))[..., 0] + nearest_world_view_transforms[:, None,
                                                                                3, :3]  # b, pts, 3
                ...
                pts_in_view_cam = pts_ @ view.world_view_transform[:3, :3] + view.world_view_transform[None, None, 3,
                                                                             :3]
                pts_projections = torch.stack(
                ...
                d_mask_all = d_mask.reshape(num_n, H, W) & (pixel_noise < 1.0) & (
                        pts_in_view_cam[..., 2].reshape(num_n, H, W) > 0.1)
            ...
            if mask_path is not None:
                mask = np.array(cv2.imread(os.path.join(mask_path, str(idx) + ".png"), cv2.IMREAD_GRAYSCALE)) > 0
                color_np = np.asarray(color)
                ref_depth = np.where(mask, ref_depth, 0)
                color_np = np.where(mask[:, :, None], color_np, 0)
                # pp=os.path.join(model_path, name, "ours_None", "renders")
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool,
                max_depth: float, voxel_size: float, use_depth_filter: bool, exclude_ply_path=None, mask_path=None):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree)
        scene = Scene(dataset, gaussians, exclude_ply_path, load_iteration=iteration, shuffle=False)
        # app_model = AppModel()
        ...
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), scene, gaussians,
                       pipeline, background,
                       max_depth=max_depth, volume=volume, use_depth_filter=use_depth_filter, mask_path=mask_path)
            print(f"extract_triangle_mesh")
            mesh = volume.extract_triangle_mesh()

            path = os.path.join(dataset.model_path, "mesh")
            os.makedirs(path, exist_ok=True)
            if exclude_ply_path is not None:
                o3d.io.write_triangle_mesh(os.path.join(path, "tsdf_fusion_" + exclude_ply_path.split('/')[-1]), mesh,
                                           write_triangle_uvs=True, write_vertex_colors=True, write_vertex_normals=True)
                mesh = clean_mesh(mesh)
                mesh.remove_unreferenced_vertices()
                mesh.remove_degenerate_triangles()
                o3d.io.write_triangle_mesh(os.path.join(path, "tsdf_fusion_post_" + exclude_ply_path.split('/')[-1]),
                                           mesh,
                                           write_triangle_uvs=True, write_vertex_colors=True, write_vertex_normals=True)
            elif mask_path is not None:
                o3d.io.write_triangle_mesh(os.path.join(path, "tsdf_fusion_mask_" + mask_path.split('/')[-2] + "_" +
                                                        mask_path.split('/')[-1] + ".ply"), mesh,
                                           write_triangle_uvs=True, write_vertex_colors=True, write_vertex_normals=True)
                mesh = clean_mesh(mesh)
                mesh.remove_unreferenced_vertices()
                mesh.remove_degenerate_triangles()
                o3d.io.write_triangle_mesh(os.path.join(path,
                                                        "tsdf_fusion_post_mask_" + mask_path.split('/')[-2] + "_" +
                                                        mask_path.split('/')[-1] + ".ply"), mesh,
                                           write_triangle_uvs=True, write_vertex_colors=True, write_vertex_normals=True)
            else:
                ...
                o3d.io.write_triangle_mesh(os.path.join(path, "tsdf_fusion_post.ply"), mesh,
                                           write_triangle_uvs=True, write_vertex_colors=True, write_vertex_normals=True)
        if not skip_test:
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), scene, gaussians,
                       pipeline, background)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exclude_ply_path", type=str, default=None, help='output mesh from 3D query')
    parser.add_argument("--mask_path", type=str, default=None, help='output mesh from 2D query')
    ...
    if hasattr(args, 'exclude_ply_path'):
        render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test,
                    args.max_depth, args.voxel_size, args.use_depth_filter, exclude_ply_path=args.exclude_ply_path)
    elif hasattr(args, 'mask_path'):
        render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test,
                    args.max_depth, args.voxel_size, args.use_depth_filter, mask_path=args.mask_path)
    else:
        render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test,
                    args.max_depth, args.voxel_size, args.use_depth_filter)
