...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool,
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(dataset.model_path, dataset.source_path, "train", scene.loaded_iter, scene.getTrainCameras(),
                       gaussians, pipeline, background, args)

        if not skip_test:
            render_set(dataset.model_path, dataset.source_path, "test", scene.loaded_iter, scene.getTestCameras(),
                       gaussians, pipeline, background, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--normalized", action="store_true",
                        help='是否归一化language_feature,避免可视化时大面积黑色的情况')
